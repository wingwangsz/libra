...
| `compat_noninteractive_history_controls` | 1 | Guards rebase autostash staged/worktree-layer terminal restoration and conflict-held GC roots, sandboxed per-commit exec retry/containment and exec-created commits, atomic update-refs with checked-out/skip/empty handling, reflog-based fork-point selection, and last-wins negative toggles | `src/command/rebase.rs`, `src/command/stash.rs`, `src/command/maintenance.rs` |
