...
| `compat_config_defaults_semantics` | 1 | Guards high-impact Git config defaults across local/global/system scopes, case-insensitive variables, empty/invalid fail-closed values, real `init.defaultBranch`/pull rebase behavior, CLI overrides, and `pull.ff=true|false|only` | `src/command/init.rs`, `src/command/pull.rs`, `src/internal/config.rs` |
