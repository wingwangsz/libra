...
- Synopsis：`libra merge [--ff | --ff-only | --no-ff] [-s ours | -X ours/theirs] [--allow-unrelated-histories] [--log[=<n>] | --no-log] [--squash | --no-commit] [-m <msg>] [--no-verify] [--verify-signatures | --no-verify-signatures] [--dry-run] <branch>` / `libra merge --continue [--no-verify]` / `libra merge --abort` / `libra merge --restart`。
