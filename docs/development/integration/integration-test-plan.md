...
| 工作区恢复与差异 | `diff`、`restore`（含真实 `--overlay`/`--no-overlay` 切换）、`reset` 的五种模式（P1-07c `--merge`/`--keep` 的精确保留/拒绝/回滚矩阵在 Cargo compat），以及 restore/reset `--pathspec-from-file` 缺失文件的负向路径 | 1 | 已实现 | Runner: `cli.restore-reset-diff`; Cargo compat: `compat_noninteractive_history_controls` |
