...
| 邮件 plumbing | P2-02 `mailinfo <msg> <patch> < mail` 的 repo-independent bounded decode、Git-shaped metadata、与 `am` 共用 subject/author/transfer cleanup、body/patch split、JSON/quiet、destination alias 与失败不覆盖旧输出 | 1 | 已实现（独立 Cargo 黑盒 target） | Cargo: `compat_mailinfo_basic`, `compat_mail_am_basic` |
