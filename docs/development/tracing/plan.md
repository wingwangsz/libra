...
| 2026-07-06 | Task C8（Code docs, compatibility and final closeout） | 进行中（session 0948cdf1 已认领；C7-independent docs 先行，待 C7 完成后终审） | — | — | in-progress 认领标记：避免并行 session 重复实施 C8。C8 依赖 C1~C7；C7（session 362f0f7a）尚在进行。策略：先落地 C7-independent 的 7 项 docs-drift 缺口（GAP-6 --goal / GAP-7 --agent / GAP-8 --approval-ttl / GAP-9 --kimi-stream / GAP-10 allow-all 五档 / GAP-11 --plan-mode 默认 / GAP-12 zh-CN 路由矩阵）到 docs/commands/code.md(+zh-CN)；待 C7 落地后补 GAP-5（run_libra_vcs allowlist 按 C7 最终 allowlist 同步）+ COMPATIBILITY.md「libra code Libra-only」+ tests/INDEX.md（含 C7 sandbox test wave/source）按 A9+C7 最终 HEAD 复核 + tracing code.md 终审一致 + fix-bridge release note（未启用→deferred/unsupported 说明）+ 全量 cargo test --all，再原子提交。完成后本行改写为验收记录 |
