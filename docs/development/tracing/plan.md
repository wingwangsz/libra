...
| 2026-07-06 | Task C3（Provider/runtime bootstrap and env handling） | 进行中（session 0948cdf1 已认领） | — | — | in-progress 认领标记：避免并行 session 重复实施 C3。范围：验证 provider factory（8 provider 默认模型/API key env/api-base 规则）、--env-file 优先于 Vault/进程环境+可操作缺 key 错误、agent profile override 不 silent fallback、Codex preflight 拒 file cwd + WebSocket timeout 诊断、live provider tests feature-gated 不入 L1——按 6 项验收核对现有代码+测试，补真实缺口。测试目标 code_provider_boot_test/code_codex_runtime_test/code_codex_default_tui_test 均已存在。完成后本行改写为验收记录 |
