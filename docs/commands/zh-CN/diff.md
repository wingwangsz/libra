...
| Minimal | | `--minimal` | 请求最短 Myers 编辑脚本，等价于 `--algorithm=myersMinimal`；不会替换显式选择的 Patience/Histogram。 |
...
选择 Myers（默认）、Myers-minimal、Patience 或 Histogram。简写形式通常更清晰；多个后端选择器同时出现时最后一个生效。`--minimal` 是独立质量请求：Myers 激活时选择 Myers-minimal，但不会覆盖显式 Patience/Histogram。
...
Myers 既是 Git 默认算法，也是 Libra 底层内建 diff 在 post-pass 前实际使用的算法。把它作为公开默认值能让文档与真实行为一致，并避免无意义的第二次 diff。Patience 适合代码块重排，Histogram 在重复文件中偏好低频锚点。Libra 的 Myers 无 deadline 且求最短编辑脚本，因此 `--minimal`/`myersMinimal` 有意与普通 Myers 输出一致，而不是静默近似。
...
| 算法 | `--algorithm=<myers|myersMinimal|patience|histogram>` 加 `--minimal`/`--patience`/`--histogram` | `--diff-algorithm`（patience/histogram/myers/minimal）及简写 | N/A（使用内部算法） |
