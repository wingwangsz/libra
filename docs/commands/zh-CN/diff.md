...
Diff 引擎当前只接入 histogram 后端。解析器为前向兼容接受 `myers` 与 `myersMinimal`，但执行时会以 `LBR-CLI-002` 显式拒绝，而不会静默换用其他算法。输出可通过 `--output` 写入文件，并提供代码评审与摘要格式（`--raw`、`--name-only`、`--name-status`、`--numstat`、`--stat`、`--compact-summary`、`--shortstat`、`--summary`）。可用 `-s`/`--no-patch` 配合 `--exit-code` 做仅状态检查；`-z`/`--null` 让 raw/name/numstat 输出以 NUL 终止，便于安全脚本解析。
